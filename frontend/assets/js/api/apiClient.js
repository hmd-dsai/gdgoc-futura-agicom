...
/**
 * Load Chat Bot feature blocks (GET /api/chatbot/features).
 * @returns {Promise<{features: Array<{id: string, title: string, status: string, detail: string}>}>}
 */
export async function getChatbotFeatures() {
    return request('/api/chatbot/features');
}

/**
 * Load quality management overview (GET /api/quality/overview).
 * @returns {Promise<{items: Array<{id: string, title: string, value: string, note: string}>}>}
 */
export async function getQualityOverview() {
    return request('/api/quality/overview');
}
