...
let appState = {
    ...
    dailySummary: null,
    /** Dynamic entries for Chat Bot pages */
    chatbotFeatures: [],
    /** Dynamic entries for Quality Management pages */
    qualityOverview: [],
};
...
async function initBackendConnection() {
    ...
    if (appState.backendOnline) {
        ...
        try {
            ...
            const chatbotPayload = await getChatbotFeatures();
            const qualityPayload = await getQualityOverview();
            appState.chatbotFeatures = chatbotPayload.features || [];
            appState.qualityOverview = qualityPayload.items || [];
...
function renderPage(pageId) {
    ...
    const titles = {
        dashboard:  { title: 'Tổng quan',                sub: 'Tổng quan chiến lược AI' },
        ...
        reviews:    { title: 'Đánh giá sản phẩm',        sub: 'Phân tích cảm xúc từ khách hàng' },
        media:      { title: 'Nội dung',                 sub: 'Hiệu suất quảng cáo và nội dung (Shopee & TikTok)' },
        chat:       { title: 'Hộp thư Chat Bot',          sub: 'Hệ thống tự động phân loại và phản hồi' },
        'chat-sentiment': { title: 'Phân tích cảm xúc',   sub: 'Theo dõi cảm xúc khách hàng từ hội thoại' },
        'chat-report':    { title: 'Báo cáo thắc mắc',    sub: 'Tổng hợp câu hỏi lặp lại theo ngữ cảnh' },
        'chat-insights':  { title: 'Tổng hợp từ chat bot', sub: 'Báo cáo tổng hợp đa phiên từ mô-đun chat bot' },
        'crisis-center':  { title: 'Quản trị khủng hoảng', sub: 'Theo dõi cảnh báo và hướng xử lý ưu tiên' },
        settings:   { title: 'Cài đặt hồ sơ',            sub: 'Cấu hình trợ lý AI' },
    };
    ...
    switch (pageId) {
        ...
        case 'chat':
            ...
            break;
        case 'chat-sentiment':
            container.innerHTML = Views.renderChatSentimentHTML(
                appState.chatbotFeatures.find(f => f.id === 'sentiment') || null,
            );
            break;
        case 'chat-report':
            container.innerHTML = Views.renderChatReportHTML(
                appState.chatbotFeatures.find(f => f.id === 'faq-report') || null,
            );
            break;
        case 'chat-insights':
            container.innerHTML = Views.renderQualityOverviewHTML(
                appState.qualityOverview.find(f => f.id === 'chatbot-summary') || null,
            );
            break;
        case 'crisis-center':
            container.innerHTML = Views.renderCrisisCenterHTML(
                appState.qualityOverview.find(f => f.id === 'crisis-management') || null,
            );
            break;
        default:
            container.innerHTML = '<div class="content-card">Tính năng đang được cập nhật...</div>';
    }
}
