...
export function renderChatSentimentHTML(feature) {
    return `
    <div class="content-card">
      <h3 class="content-card-title">Phân tích cảm xúc</h3>
      <div class="grid-2">
        <div class="stat-card emerald">
          <div class="stat-card-label">Trạng thái hệ thống</div>
          <div class="stat-card-value">${feature?.status || 'Đang đồng bộ'}</div>
        </div>
        <div class="stat-card">
          <div class="stat-card-label">Ghi chú</div>
          <div style="font-size:0.9rem;color:var(--text-secondary);margin-top:8px;">
            ${feature?.detail || 'Phân tích cảm xúc đang lấy dữ liệu từ hội thoại gần nhất.'}
          </div>
        </div>
      </div>
    </div>
...
export function renderChatReportHTML(feature) {
    return `
    <div class="content-card">
      <h3 class="content-card-title">Báo cáo thắc mắc</h3>
      <div style="font-size:0.9rem;color:var(--text-secondary);line-height:1.7;">
        ${feature?.detail || 'Hệ thống tổng hợp câu hỏi thường gặp theo thời gian thực.'}
      </div>
      <div style="margin-top:14px;padding:12px;background:var(--bg-glass);border-radius:8px;">
        <strong>Trạng thái:</strong> ${feature?.status || 'Đang cập nhật'}
      </div>
    </div>
  `;
}

export function renderQualityOverviewHTML(item) {
    return `
    <div class="content-card">
      <h3 class="content-card-title">Tổng hợp từ chat bot</h3>
      <div class="stats-grid" style="grid-template-columns:1fr 1fr;">
        <div class="stat-card indigo">
          <div class="stat-card-label">Chỉ số tổng hợp</div>
          <div class="stat-card-value">${item?.value || '0'}</div>
        </div>
        <div class="stat-card">
          <div class="stat-card-label">Ghi chú vận hành</div>
          <div style="font-size:0.85rem;color:var(--text-secondary);margin-top:10px;">
            ${item?.note || 'Dữ liệu tổng hợp từ các luồng hội thoại đã duyệt.'}
          </div>
        </div>
      </div>
    </div>
...
export function renderCrisisCenterHTML(item) {
    return `
    <div class="content-card">
      <h3 class="content-card-title">Quản trị khủng hoảng</h3>
      <div style="display:flex;align-items:flex-start;gap:12px;">
        <div style="font-size:1.3rem;">🚨</div>
        <div>
          <div style="font-weight:700;color:var(--text-heading);">${item?.value || '0 cảnh báo mới'}</div>
          <div style="font-size:0.88rem;color:var(--text-secondary);margin-top:6px;line-height:1.7;">
            ${item?.note || 'Không có sự cố nghiêm trọng trong chu kỳ hiện tại.'}
          </div>
        </div>
      </div>
    </div>
